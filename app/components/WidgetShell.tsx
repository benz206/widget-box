...
export function WidgetShell({ title, subtitle, size, children }: WidgetShellProps) {
  return (
    <section
      className={[
    ...
    >
      ...
      {(title || subtitle) && (
        <header className="z-10 flex items-baseline justify-between mb-2">
          <div>
            {title && <h3 className="text-sm font-semibold drop-shadow-[0_1px_0_rgba(255,255,255,.3)]">{title}</h3>}
            {subtitle && <p className="text-xs text-white/80 dark:text-neutral-200/70">{subtitle}</p>}
          </div>
        </header>
      )}
